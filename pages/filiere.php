...
try {
    ...
    if (isset($_SESSION['user'])) {
        ...
        $ofset = ($page - 1) * $size;
        // recuperation de toutes les filiere
            ...
            <div class="panel-body">
                ...
                <form action="filiere.php" method="get" class="form-inline">
                    <div class="form-group">
                        <input type="text" name="valeurNomUser" placeholder="tapper le nom de votre filiere" class="form-control" value="<?php $nom; ?>">
                    </div>
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <?php while ($filiere = $resultatFiliere->fetch()) { ?>
                        <tr>
                            ...
                            <?php if ($_SESSION['user']['role'] === 'ADMIN') { ?>
                                <td>
                                    <a onclick="return confirm('Voulez vous vraiment modifier cette information?')" href="editerFiliere.php?idF= <?php echo $filiere['idFiliere'] ?>" class="glyphicon glyphicon-edit"></a>
                                    <a onclick="return confirm('Etes vous sur de vouloir supprimer cette information?')" href="supprimerFiliere.php?idF= <?php echo $filiere['idFiliere'] ?>" class="glyphicon glyphicon-trash"></a>
                                <?php } ?>
                                </td>
                            <?php } ?>
                        </tr>
                    ...
                    <ul class="pagination pagination-sm">
                        <?php for ($i = 1; $i <= $nombrePage; $i++) { ?>
                            <li class="<?php if ($i == $page)
                                            echo 'active'; ?>"><a href="filiere.php?page=<?php echo $i; ?>&nom=<?php echo $nom ?>&niveau=<?php echo $niveau ?>">
                                    <?php echo $i; ?>
                                </a></li>
